...
class Renderer:
    ...
    def render(self, lines, override_css_params=None):
        ...
        # If text rendering failed, propagate the failure
        if img is None:
            return None, params
    ...
    def render_text(self, lines, override_css_params=None):
        ...
        # rough size estimate; image is cropped later anyway
        max_chars = max(len(line) for line in lines) if lines else 1
        num_lines = max(len(lines), 1)
        fs = params["font_size"]

        if params["vertical"]:
            # For vertical text: height grows with characters in the longest line,
            # width grows with number of lines.
            width = int(num_lines * fs * 2.5)
            height = int(max_chars * fs * 2.5)
        else:
            # For horizontal text: width grows with longest line, height with lines.
            width = int(max_chars * fs * 2.5)
            height = int(num_lines * fs * 2.5)

        # Add a minimum size so short texts still have margin
        width = max(width, 512)
...
"""
        ...
        html_path = (self.temp_dir / html_name).resolve()
        png_path = (self.temp_dir / png_name).resolve()

        try:
            # Write HTML file
            html_path.write_text(html_doc, encoding="utf-8")

            # Build a proper file:// URL, e.g. file:///home/.../foo.html
            file_url = html_path.as_uri()

            # Use screenshot_url with the file:// URL
            self.hti.screenshot_url(
                url=file_url,
            ...
            if img is None:
                print(f"[warn] Failed to read image from {png_path}")
                return None, params

            # Ensure we have an alpha channel for later crop_by_alpha
    ...
    def get_random_css_params(self, font_path=None):
        ...
        # Add text alignment variation for horizontal text
        if not params["vertical"]:
    ...
    def render_background(self, img):
        ...
        # If no backgrounds available, just crop by alpha and return
        if self.background_df is None or self.background_df.empty:
        ...
        if not draw_bubble:
            # When no bubble, ALWAYS invert text for better contrast
            # Changed from 50% to 100% chance
            img[:, :, :3] = 255 - img[:, :, :3]
...
def get_css(
    ...
    text_align=None,  # Add this parameter
):
    styles = [
